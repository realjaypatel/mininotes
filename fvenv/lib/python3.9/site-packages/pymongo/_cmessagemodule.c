...
static int buffer_write_bytes_ssize_t(buffer_t buffer, const char* data,
                                      Py_ssize_t size) {
...
static PyObject* _cbson_query_message(PyObject* self, PyObject* args) {
  ...
  if (!(PyArg_ParseTuple(args, "Iet#iiOOO", &flags, "utf-8", &collection_name,
                         &collection_name_length, &num_to_skip, &num_to_return,
                         &query, &field_selector, &options_obj) &&
  ...
  if (field_selector != Py_None) {
    begin = pymongo_buffer_get_position(buffer);
    if (!write_dict(state->_cbson, buffer, field_selector, 0, &options, 1)) {
      goto fail;
    }
  ...
  buffer_write_int32_at_position(buffer, length_location,
                                 (int32_t)message_length);

  /* objectify buffer */
  result =
      Py_BuildValue("iy#i", request_id, pymongo_buffer_get_buffer(buffer),
                    (Py_ssize_t)pymongo_buffer_get_position(buffer), max_size);
fail:
...
static PyObject* _cbson_get_more_message(PyObject* self, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "et#iL", "utf-8", &collection_name,
                        &collection_name_length, &num_to_return, &cursor_id)) {
    return NULL;
  }
  buffer = pymongo_buffer_new();
  if (!buffer) {
  ...
  /* objectify buffer */
  result = Py_BuildValue("iy#", request_id, pymongo_buffer_get_buffer(buffer),
...
static PyObject* _cbson_op_msg(PyObject* self, PyObject* args) {
  ...
  if (!(PyArg_ParseTuple(args, "IOet#OO", &flags, &command, "utf-8",
  ...
  total_size = write_dict(state->_cbson, buffer, command, 0, &options, 1);
  if (!total_size) {
    goto fail;
  }
  ...
  if (identifier_length) {
    ...
    /* C string identifier */
    if (!buffer_write_bytes_ssize_t(buffer, identifier,
                                    identifier_length + 1)) {
      goto fail;
    }
    ...
    while ((doc = PyIter_Next(iterator)) != NULL) {
      int encoded_doc_size =
          write_dict(state->_cbson, buffer, doc, 0, &options, 1);
      if (!encoded_doc_size) {
        Py_CLEAR(doc);
        goto fail;
      }
    ...
    payload_length =
        pymongo_buffer_get_position(buffer) - payload_one_length_location;
    buffer_write_int32_at_position(buffer, payload_one_length_location,
  ...
  /* objectify buffer */
  result = Py_BuildValue("iy#ii", request_id, pymongo_buffer_get_buffer(buffer),
                         (Py_ssize_t)pymongo_buffer_get_position(buffer),
                         total_size, max_doc_size);
fail:
  Py_XDECREF(iterator);
  if (buffer) {
    pymongo_buffer_free(buffer);
...
static void _set_document_too_large(int size, long max) {
...
static int _batched_op_msg(unsigned char op, unsigned char ack,
  ...
  max_write_batch_size_obj =
      PyObject_GetAttr(ctx, state->_max_write_batch_size_str);
  ...
  if (!write_dict(state->_cbson, buffer, command, 0, &options, 0)) {
    return 0;
  }
  ...
  switch (op) {
    case _INSERT: {
      if (!buffer_write_bytes(buffer, "documents\x00", 10)) goto fail;
      break;
    }
    case _UPDATE: {
      if (!buffer_write_bytes(buffer, "updates\x00", 8)) goto fail;
      break;
    }
    case _DELETE: {
      if (!buffer_write_bytes(buffer, "deletes\x00", 8)) goto fail;
      break;
    }
    default: {
  ...
  while ((doc = PyIter_Next(iterator)) != NULL) {
    ...
    doc_too_large =
        (idx == 0 && (pymongo_buffer_get_position(buffer) > max_message_size));
    ...
    if (doc_too_large || unacked_doc_too_large) {
      if (op == _INSERT) {
        ...
        if (DocumentTooLarge) {
           ...
           */
          PyErr_Format(DocumentTooLarge, "%s command document too large",
...
static PyObject* _cbson_encode_batched_op_msg(PyObject* self, PyObject* args) {
  unsigned char op;
  unsigned char ack;
  PyObject* command = NULL;
  PyObject* docs = NULL;
  ...
  if (!(PyArg_ParseTuple(args, "bOObOO", &op, &command, &docs, &ack,
                         &options_obj, &ctx) &&
        convert_codec_options(state->_cbson, options_obj, &options))) {
    return NULL;
  }
  ...
  if (!_batched_op_msg(op, ack, command, docs, ctx, to_publish, options, buffer,
                       state)) {
    goto fail;
  }

  result = Py_BuildValue("y#O", pymongo_buffer_get_buffer(buffer),
                         (Py_ssize_t)pymongo_buffer_get_position(buffer),
                         to_publish);
fail:
  destroy_codec_options(&options);
...
static PyObject* _cbson_batched_op_msg(PyObject* self, PyObject* args) {
  unsigned char op;
  unsigned char ack;
  int request_id;
  int position;
  ...
  result = Py_BuildValue("iy#O", request_id, pymongo_buffer_get_buffer(buffer),
                         (Py_ssize_t)pymongo_buffer_get_position(buffer),
                         to_publish);
fail:
  destroy_codec_options(&options);
...
static int _batched_write_command(char* ns, Py_ssize_t ns_len, unsigned char op,
                                  PyObject* command, PyObject* docs,
                                  PyObject* ctx, PyObject* to_publish,
                                  codec_options_t options, buffer_t buffer,
  ...
  if (!buffer_write_bytes(buffer, "\x00\x00\x00\x00", /* flags */
                          4) ||
  ...
  *(pymongo_buffer_get_buffer(buffer) +
    (pymongo_buffer_get_position(buffer) - 1)) = 0x4;
  ...
  while ((doc = PyIter_Next(iterator)) != NULL) {
    ...
    enough_data =
        (idx >= 1 && (pymongo_buffer_get_position(buffer) > max_split_size));
    if (enough_data) {
      /*
       * Roll the existing buffer back to the beginning
       * of the last document encoded.
...
static PyObject* _cbson_encode_batched_write_command(PyObject* self,
                                                     PyObject* args) {
  char* ns = NULL;
  unsigned char op;
  Py_ssize_t ns_len;
  ...
  if (!(PyArg_ParseTuple(args, "et#bOOOO", "utf-8", &ns, &ns_len, &op, &command,
                         &docs, &options_obj, &ctx) &&
        convert_codec_options(state->_cbson, options_obj, &options))) {
    return NULL;
  }
  if (!(buffer = pymongo_buffer_new())) {
  ...
  if (!_batched_write_command(ns, ns_len, op, command, docs, ctx, to_publish,
                              options, buffer, state)) {
    goto fail;
  }
...
static PyMethodDef _CMessageMethods[] = {
    ...
    {"_encode_batched_write_command", _cbson_encode_batched_write_command,
     METH_VARARGS, "Encode the next batched insert, update, or delete command"},
 ...
 */
static int _cmessage_exec(PyObject* m) {
  ...
  if (!((state->_max_bson_size_str = PyUnicode_FromString("max_bson_size")) &&
        (state->_max_message_size_str =
             PyUnicode_FromString("max_message_size")) &&
        (state->_max_write_batch_size_str =
             PyUnicode_FromString("max_write_batch_size")) &&
        (state->_max_split_size_str =
             PyUnicode_FromString("max_split_size")))) {
    goto fail;
  }
...
static struct PyModuleDef moduledef = {
    PyModuleDef_HEAD_INIT,       "_cmessage",      NULL,
    sizeof(struct module_state), _CMessageMethods, _cmessage_slots,
    _cmessage_traverse,          _cmessage_clear,  NULL};

PyMODINIT_FUNC PyInit__cmessage(void) { return PyModuleDef_Init(&moduledef); }
