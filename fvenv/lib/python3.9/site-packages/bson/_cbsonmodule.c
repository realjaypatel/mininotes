...
static PyObject* _test_long_long_to_str(PyObject* self, PyObject* args) {
  ...
  if (strcmp(str_1, str_2) != 0) {
    PyErr_Format(PyExc_RuntimeError, "LL2STR != INT2STRING: %s != %s", str_1,
                 str_2);
    return NULL;
  }
...
static int _downcast_and_check(Py_ssize_t size, uint8_t extra) {
  if (size > BSON_MAX_SIZE || ((BSON_MAX_SIZE - extra) < size)) {
    PyObject* InvalidStringData = _error("InvalidStringData");
    if (InvalidStringData) {
      PyErr_SetString(InvalidStringData, "String length must be <= 2147483647");
...
static PyObject* datetime_from_millis(long long millis) {
  /* To encode a datetime instance like datetime(9999, 12, 31, 23, 59, 59,
   * 999999) we follow these steps:
   ...
   * Resulting in datetime(9999, 12, 31, 23, 59, 59, 999000) -- the expected
   * result
   *
  ...
  if (!datetime) {
    ...
    if (PyErr_GivenExceptionMatches(etype, PyExc_ValueError)) {
      if (evalue) {
        ...
        if (err_msg) {
          PyObject* appendage = PyUnicode_FromString(
              " (Consider Using CodecOptions(datetime_conversion=DATETIME_AUTO) or MongoClient(datetime_conversion='DATETIME_AUTO')). See: https://www.mongodb.com/docs/languages/python/pymongo-driver/current/data-formats/dates-and-times/#handling-out-of-range-datetimes");
          if (appendage) {
...
/* Extended-range datetime, takes a DatetimeMS object and extracts the long long
 * value. */
...
static PyObject* decode_datetime(PyObject* self, long long millis,
                                 const codec_options_t* options) {
  PyObject* naive = NULL;
  ...
  if (dt_clamp || dt_auto) {
    ...
    if (options->tz_aware && options->tzinfo && options->tzinfo != Py_None) {
      ...
      PyObject* utcoffset = PyObject_VectorcallMethod(state->_utcoffset_str,
                                                      utcoffset_args, 2, NULL);
      if (utcoffset == NULL) {
        return 0;
      }
      if (utcoffset != Py_None) {
        if (!PyDelta_Check(utcoffset)) {
          ...
          if (BSONError) {
            PyErr_SetString(
                BSONError,
                "tzinfo.utcoffset() did not return a datetime.timedelta");
            Py_DECREF(BSONError);
          }
          Py_DECREF(utcoffset);
          return 0;
        ...
        min_millis_offset =
            (PyDateTime_DELTA_GET_DAYS(utcoffset) * (int64_t)86400 +
      ...
      utcoffset = PyObject_VectorcallMethod(state->_utcoffset_str,
                                            utcoffset_args, 2, NULL);
      if (utcoffset == NULL) {
        return 0;
      }
  ...
  if (options->tzinfo != Py_None) {
    ...
    PyObject* temp = PyObject_VectorcallMethod(state->_astimezone_str,
                                               astimezone_args, 2, NULL);
    Py_DECREF(value);
    value = temp;
  }
invalid:
...
void buffer_write_int32_at_position(buffer_t buffer, int position,
                                    int32_t data) {
  uint32_t data_le = BSON_UINT32_TO_LE(data);
...
static int write_unicode(buffer_t buffer, PyObject* py_string) {
  ...
  if (!data) goto unicodefail;

  if ((size = _downcast_and_check(PyBytes_GET_SIZE(encoded), 1)) == -1)
    goto unicodefail;

  if (!buffer_write_int32(buffer, (int32_t)size)) goto unicodefail;

  if (!buffer_write_bytes(buffer, data, size)) goto unicodefail;
...
static int write_string(buffer_t buffer, PyObject* py_string) {
  ...
  if ((size = _downcast_and_check(PyBytes_Size(py_string), 1)) == -1) return 0;
...
static int _load_object(PyObject** object, char* module_name,
                        char* object_name) {
  PyObject* module;
...
static int _load_python_objects(PyObject* module) {
  ...
  if (!((state->_type_marker_str = PyUnicode_FromString("_type_marker")) &&
        ...
        (state->_fallback_encoder_str =
             PyUnicode_FromString("_fallback_encoder")) &&
  ...
  if (PyDict_SetItem(state->replace_kwargs, state->_tzinfo_str, state->UTC) ==
      -1) {
    return 1;
  }
 ...
 * Return 0 on failure.
 */
int cbson_convert_type_registry(PyObject* registry_obj,
  ...
  registry->fallback_encoder =
      PyObject_GetAttr(registry_obj, _fallback_encoder_str);
 ...
 * Return 0 on failure.
 */
int convert_codec_options(PyObject* self, PyObject* options_obj,
  ...
  if (!PyArg_ParseTuple(options_obj, "ObbzOOb", &options->document_class,
                        &options->tz_aware, &options->uuid_rep,
                        &options->unicode_decode_error_handler,
                        &options->tzinfo, &type_registry_obj,
                        &options->datetime_conversion)) {
    return 0;
  }

  type_marker = _type_marker(options->document_class, state->_type_marker_str);
  if (type_marker < 0) {
    return 0;
  }

  if (!cbson_convert_type_registry(
          type_registry_obj, &options->type_registry, state->_encoder_map_str,
          state->_decoder_map_str, state->_fallback_encoder_str)) {
    return 0;
  }
...
static int write_element_to_buffer(PyObject* self, buffer_t buffer,
  ...
  result = _write_element_to_buffer(self, buffer, type_byte, value, check_keys,
                                    options, in_custom_call, in_fallback_call);
  Py_LeaveRecursiveCall();
  return result;
}

static void _set_cannot_encode(PyObject* value) {
  ...
  PyErr_Format(InvalidDocument, "cannot encode object: %R, of type: %R", value,
               type);
error:
  Py_XDECREF(type);
 ...
 */
static int _write_regex_to_buffer(buffer_t buffer, int type_byte,
                                  PyObject* value, PyObject* _flags_str,
                                  PyObject* _pattern_str) {
  PyObject* py_flags;
  PyObject* py_pattern;
  ...
  if ((pattern_length =
           _downcast_and_check(PyBytes_Size(encoded_pattern), 0)) == -1) {
    Py_DECREF(encoded_pattern);
    return 0;
  }
  ...
  if (check_utf8) {
    decoded_pattern =
        PyUnicode_DecodeUTF8(pattern_data, (Py_ssize_t)pattern_length, NULL);
...
static int _write_element_to_buffer(PyObject* self, buffer_t buffer,
  ...
  if (PyUnicode_CheckExact(value) || PyLong_CheckExact(value) ||
      PyFloat_CheckExact(value) || PyBool_Check(value) ||
      PyDict_CheckExact(value) || PyList_CheckExact(value) ||
      PyTuple_CheckExact(value) || PyBytes_CheckExact(value) ||
      value == Py_None) {
    type = 0;
  } else {
  ...
  switch (type) {
    ...
    case 11: {
      /* Regex */
      return _write_regex_to_buffer(buffer, type_byte, value, state->_flags_str,
                                    state->_pattern_str);
    }
    case 13: {
      /* Code */
      ...
      *(pymongo_buffer_get_buffer(buffer) + type_byte) = 0x0F;
      ...
      buffer_write_int32_at_position(buffer, length_location, (int32_t)length);
      return 1;
    }
    case 17: {
      /* Timestamp */
    ...
    case 100: {
      ...
      PyObject* as_doc =
          PyObject_VectorcallMethod(state->_as_doc_str, as_doc_args, 1, NULL);
      if (!as_doc) {
        return 0;
      }
  ...
  if (PyBool_Check(value)) {
    ...
    *(pymongo_buffer_get_buffer(buffer) + type_byte) = 0x04;
    ...
    if (items > BSON_MAX_SIZE) {
      PyObject* BSONError = _error("BSONError");
      if (BSONError) {
        PyErr_SetString(BSONError, "Too many items to serialize.");
        Py_DECREF(BSONError);
      }
      return 0;
    }
    for (i = 0; i < items; i++) {
      ...
      if (!write_element_to_buffer(self, buffer, list_type_byte, item_value,
                                   check_keys, options, 0, 0)) {
        return 0;
      }
    }
    ...
    *(pymongo_buffer_get_buffer(buffer) + type_byte) = 0x02;
    ...
    PyObject* utcoffset = PyObject_VectorcallMethod(state->_utcoffset_str,
                                                    utcoffset_args, 1, NULL);
    if (utcoffset == NULL) return 0;
    if (utcoffset != Py_None) {
    ...
    *(pymongo_buffer_get_buffer(buffer) + type_byte) = 0x03;
    ...
    binary_value = PyObject_VectorcallMethod(state->_from_uuid_str,
                                             from_uuid_args, 3, NULL);
    ...
    result = _write_element_to_buffer(self, buffer, type_byte, binary_value,
                                      check_keys, options, in_custom_call,
                                      in_fallback_call);
  ...
  if (!in_fallback_call && options->type_registry.has_fallback_encoder) {
    ...
    new_value = PyObject_Vectorcall(options->type_registry.fallback_encoder,
...
static int check_key_name(const char* name, int name_length) {
  if (name_length > 0 && name[0] == '$') {
    ...
    if (InvalidDocument) {
      PyObject* errmsg =
          PyUnicode_FromFormat("key '%s' must not start with '$'", name);
      if (errmsg) {
  ...
  if (strchr(name, '.')) {
    ...
    if (InvalidDocument) {
      PyObject* errmsg =
          PyUnicode_FromFormat("key '%s' must not contain '.'", name);
      if (errmsg) {
        PyErr_SetObject(InvalidDocument, errmsg);
        Py_DECREF(errmsg);
      }
 ...
 *
 * Returns 0 on failure */
int write_pair(PyObject* self, buffer_t buffer, const char* name,
               int name_length, PyObject* value, unsigned char check_keys,
  ...
  if (!write_element_to_buffer(self, buffer, type_byte, value, check_keys,
                               options, 0, 0)) {
    return 0;
  }
  return 1;
...
int decode_and_write_pair(PyObject* self, buffer_t buffer, PyObject* key,
  ...
  /* If top_level is True, don't allow writing _id here - it was already
   * written. */
  if (!write_pair(self, buffer, data, size - 1, value, check_keys, options,
                  !top_level)) {
    Py_DECREF(encoded);
    return 0;
  }
...
int write_dict(PyObject* self, buffer_t buffer, PyObject* dict,
               unsigned char check_keys, const codec_options_t* options,
               unsigned char top_level) {
  PyObject* key;
  PyObject* iter;
  char zero = 0;
  ...
  if (!is_dict) {
    ...
    if (!PyObject_IsInstance(dict, state->Mapping)) {
      ...
      if ((repr = PyObject_Repr(dict))) {
        PyObject* errmsg =
            PyUnicode_FromString("encoder expected a mapping type but got: ");
        if (errmsg) {
          PyObject* error = PyUnicode_Concat(errmsg, repr);
          if (error) {
        ...
        PyErr_SetString(PyExc_TypeError, "encoder expected a mapping type");
      }
  ...
  if (top_level) {
    ...
    if (is_dict) {
      ...
      if (_id) {
        if (!write_pair(self, buffer, "_id", 3, _id, check_keys, options, 1)) {
          return 0;
        }
      }
  ...
  if (is_dict) {
    ...
    while (PyDict_Next(dict, &pos, &key, &value)) {
      if (!decode_and_write_pair(self, buffer, key, value, check_keys, options,
                                 top_level)) {
...
static PyObject* _cbson_dict_to_bson(PyObject* self, PyObject* args) {
  ...
  if (!(PyArg_ParseTuple(args, "ObO|b", &dict, &check_keys, &options_obj,
                         &top_level) &&
...
static PyObject* _dbref_hook(PyObject* self, PyObject* value) {
  ...
  /* Decoding for DBRefs */
  if (PyMapping_HasKey(value, state->_dollar_ref_str) &&
    ...
    if (!(PyUnicode_Check(ref) &&
          (database == Py_None || PyUnicode_Check(database)))) {
      ret = value;
      goto invalid;
    }
...
static PyObject* get_value(PyObject* self, PyObject* name, const char* buffer,
                           ...
                           const codec_options_t* options, int raw_array) {
  struct module_state* state = GETSTATE(self);
  PyObject* value = NULL;
  if (!state) {
    return NULL;
  ...
  switch (type) {
    ...
    case 14: {
      ...
      *position += 4;
      ...
      value = PyUnicode_DecodeUTF8(buffer + *position, value_length - 1,
                                   options->unicode_decode_error_handler);
      if (!value) {
        goto invalid;
      }
    ...
    case 3: {
      ...
      value = elements_to_dict(self, buffer + *position, size, options);
      if (!value) {
        goto invalid;
      }
      ...
      *position += 5;
      ...
      if (subtype == 3 || subtype == 4) {
        ...
        if ((uuid_rep == UNSPECIFIED) ||
          ...
          value = PyObject_VectorcallMethod(state->_as_uuid_str, as_uuid_args,
                                            2, NULL);
          Py_DECREF(uuid_rep_obj);
        }
    ...
    case 11: {
      ...
      pattern = PyUnicode_DecodeUTF8(buffer + *position, pattern_length,
                                     options->unicode_decode_error_handler);
      if (!pattern) {
        goto invalid;
      }
    ...
    case 12: {
      ...
      *position += 4;
      ...
      collection = PyUnicode_DecodeUTF8(buffer + *position, coll_length - 1,
                                        options->unicode_decode_error_handler);
      if (!collection) {
        goto invalid;
      }
    ...
    case 13: {
      ...
      *position += 4;
      ...
      code = PyUnicode_DecodeUTF8(buffer + *position, value_length - 1,
                                  options->unicode_decode_error_handler);
      if (!code) {
        goto invalid;
      }
    ...
    case 15: {
      ...
      *position += 4;
      ...
      code = PyUnicode_DecodeUTF8(buffer + *position, code_size - 1,
                                  options->unicode_decode_error_handler);
      if (!code) {
        goto invalid;
      }
      *position += code_size;
      ...
      scope = elements_to_dict(self, buffer + *position, scope_size, options);
      if (!scope) {
        Py_DECREF(code);
        goto invalid;
      }
    ...
    case 19: {
      ...
      PyObject* _bytes_obj =
          PyBytes_FromStringAndSize(buffer + *position, (Py_ssize_t)16);
      if (!_bytes_obj) {
        goto invalid;
      }
      PyObject* dec128_args[2] = {state->Decimal128, _bytes_obj};
      value =
          PyObject_VectorcallMethod(state->_from_bid_str, dec128_args, 2, NULL);
    ...
    default: {
      ...
      if (InvalidBSON) {
        ...
        if (bobj) {
          ...
          if (repr) {
            PyObject* left =
                PyUnicode_FromString("Detected unknown BSON type ");
            if (left) {
  ...
  if (value) {
    if (!options->type_registry.is_decoder_empty) {
      ...
      converter =
          PyDict_GetItem(options->type_registry.decoder_map, value_type);
      if (converter != NULL) {
        PyObject* converter_args[1] = {value};
        PyObject* new_value =
            PyObject_Vectorcall(converter, converter_args, 1, NULL);
...
static int _element_to_dict(PyObject* self, const char* string,
  ...
  *name = PyUnicode_DecodeUTF8(string + position, name_length,
                               options->unicode_decode_error_handler);
  if (!*name) {
  ...
  *value = get_value(self, *name, string, &position, type, max - position,
                     options, raw_array);
  if (!*value) {
    Py_DECREF(*name);
    return -1;
...
static PyObject* _cbson_element_to_dict(PyObject* self, PyObject* args) {
  ...
  if (!(PyArg_ParseTuple(args, "OIIOp", &bson, &position, &max, &options_obj,
                         &raw_array) &&
        convert_codec_options(self, options_obj, &options))) {
    return NULL;
  }
  ...
  if (!PyBytes_Check(bson)) {
    PyErr_SetString(PyExc_TypeError,
                    "argument to _element_to_dict must be a bytes object");
    return NULL;
  }
...
static PyObject* elements_to_dict(PyObject* self, const char* string,
  ...
  if (Py_EnterRecursiveCall(" while decoding a BSON document")) return NULL;
...
static int _get_buffer(PyObject* exporter, Py_buffer* view) {
  ...
  if (!PyBuffer_IsContiguous(view, 'C')) {
    PyErr_SetString(PyExc_ValueError, "must be a contiguous buffer");
    goto fail;
  }
  ...
  if (view->itemsize != 1) {
    PyErr_SetString(PyExc_ValueError, "buffer data must be ascii or utf8");
    goto fail;
  }
  return 1;
fail:
...
static PyObject* _cbson_array_of_documents_to_buffer(PyObject* self,
                                                     PyObject* args) {
  uint32_t size;
  uint32_t value_length;
  uint32_t position = 0;
...
static PyMethodDef _CBSONMethods[] = {
    ...
    {"_array_of_documents_to_buffer", _cbson_array_of_documents_to_buffer,
     METH_VARARGS,
     "Convert raw array of documents to a stream of BSON documents"},
    {"_test_long_long_to_str", _test_long_long_to_str, METH_VARARGS,
     "Test conversion of extreme and common Py_ssize_t values to str."},
    {NULL, NULL, 0, NULL}};
 ...
 */
static int _cbson_exec(PyObject* m) {
  ...
  if (c_api_object == NULL) INITERROR;
...
PyMODINIT_FUNC PyInit__cbson(void) { return PyModuleDef_Init(&moduledef); }
